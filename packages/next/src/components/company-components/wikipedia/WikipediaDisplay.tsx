...
const extractCompanyName = (url: string): string => {
  try {
    ...
    // Create URL object to properly parse the domain
    const urlObj = new URL(url.startsWith('http') ? url : `https://${url}`);
    
    // Get hostname and remove www if present
    const hostname = urlObj.hostname.replace(/^www\./, '');
    
    // Split by dots and get the domain part
    const parts = hostname.split('.');
    
    // Handle subdomains by taking the last meaningful part before TLD
    // e.g., company.co.uk -> company, sub.company.com -> company
...
const WikipediaDisplay: React.FC<WikipediaDisplayProps> = ({ data, websiteUrl }) => {
  ...
  if (!data || (!data.text && !data.url) || (Array.isArray(data) && data.length === 0) || 
      (Object.prototype.hasOwnProperty.call(data, 'results') && Array.isArray((data as any).results) && (data as any).results.length === 0)) {
    return null;
  }
